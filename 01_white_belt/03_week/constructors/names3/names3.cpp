...
class Person
{
	...
	private:
		...
		string	Find(int year, const map<int, string>& dict, const vector<int>& v) const
		{
			if (dict.count(year))
				return (dict.at(year));
			int	max = -1;
		...
		string	History(int year, const string& name, const map<int, string>& dict, const vector<int>& v) const
		{
		...
		string	FindFirstName(int year) const
		{
			return (Find(year, first_name_dict, first_name_years));
		}

		string	FindLastName(int year) const
		{
			return (Find(year, last_name_dict, last_name_years));
		}

		string	HistoryFirstName(int year, const string& first_name) const
		{
			return (History(year, first_name, first_name_dict, first_name_years));
		}
		
		string	HistoryLastName(int year, const string& last_name) const
		{
			return (History(year, last_name, last_name_dict, last_name_years));
		}

		string	JoinNameHistory(const string& name, const string &history) const
		{
			if (history == "")
				return (name);
			string	out = name;
		...
		string	JoinFullNameHistory(const string& first_name, const string& last_name,
					const string& first_name_history, const string& last_name_history) const
		{
